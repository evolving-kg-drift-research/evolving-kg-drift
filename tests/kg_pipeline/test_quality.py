...
def test_evaluate_quality_empty_gold():
    fact1 = FactVersion(
        fact_version_id="f1", logical_fact_id="lf1",
        subject_id="A", relation_id="REL", object_id="B",
        valid_from=dt(2021,1,1), valid_to=None, evidence_observed_at=dt(2021,1,1),
    ...
    # P1 fix: Quality check MUST FAIL when evaluated_count is 0 or gold is empty
    report = evaluate_quality([fact1], [], error_threshold=0.0)
    assert report["status"] == "FAIL"
