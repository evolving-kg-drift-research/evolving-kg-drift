...
class AlignmentDiagnostics:
    ...
    def check_alignment_validity(
        self,
        min_numerical_rank: int | None = None,
        max_fit_holdout_gap: float = 1.0,
        max_holdout_residual: float = 2.0,
        max_orthogonality_error: float = 1e-4,
    ) -> tuple[bool, str]:
        """Validates whether the Procrustes alignment satisfies operating constraints (A24).

        Returns (is_valid, reason_if_invalid).
        """
        required_rank = (
            min_numerical_rank
            if min_numerical_rank is not None
            else max(1, self.dimension // 2)
        )
        if self.numerical_rank < required_rank:
            return False, f"RANK_DEFICIENT:{self.numerical_rank}<{required_rank}"

        if self.orthogonality_error > max_orthogonality_error:
            return False, f"ORTHOGONALITY_VIOLATION:{self.orthogonality_error}>{max_orthogonality_error}"

        if self.fit_holdout_gap > max_fit_holdout_gap:
            return False, f"EXCESSIVE_FIT_HOLDOUT_GAP:{self.fit_holdout_gap}>{max_fit_holdout_gap}"

        if self.holdout_residual > max_holdout_residual:
            return False, f"EXCESSIVE_HOLDOUT_RESIDUAL:{self.holdout_residual}>{max_holdout_residual}"

        return True, ""

    @property
    def is_valid(self) -> bool:
        valid, _ = self.check_alignment_validity()
        return valid
...
class ProcrustesAlignmentResult:
    ...
    def align_entity_vector_centered(self, x: Sequence[float]) -> list[float]:
        r"""Aligns source entity vector without re-adding target mean: x'_c = (x - \mu_{source}) Q."""
        x_c = [x[i] - self.source_mean[i] for i in range(self.dimension)]
        return [
            sum(x_c[i] * self.Q[i][j] for i in range(self.dimension))
            for j in range(self.dimension)
    ...
    def center_target_vector(self, y: Sequence[float]) -> list[float]:
        r"""Centers target entity vector: y_c = y - \mu_{target}."""
        return [y[i] - self.target_mean[i] for i in range(self.dimension)]
    ...
    def align_all_entities_centered(
        self,
        entity_embeddings: dict[str, list[float]],
    ) -> dict[str, list[float]]:
        r"""Aligns all entity embeddings centered: (x - \mu_{source}) Q."""
        return {
            e: self.align_entity_vector_centered(vec)
            for e, vec in entity_embeddings.items()
        }

    def center_all_target_entities(
        self,
        entity_embeddings: dict[str, list[float]],
    ) -> dict[str, list[float]]:
        r"""Centers all target entity embeddings: y - \mu_{target}."""
        return {
            e: self.center_target_vector(vec)
            for e, vec in entity_embeddings.items()
        }
