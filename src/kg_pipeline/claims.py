...
def extract_claims(
    ...
    system_prompt = "You are a JSON-only extraction bot."
    config = {"temperature": adapter.temperature, "base_url": getattr(adapter, "base_url", None)}
    cache_key = get_cache_key(prompt, adapter.model, config, system_prompt=system_prompt)
    ...
    for i, rc in enumerate(raw_claims):
        try:
            ...
            if not isinstance(start_idx, int) or not isinstance(end_idx, int) or isinstance(start_idx, bool) or isinstance(end_idx, bool):
                raise ContractError("Evidence span offsets must be integers.")
            ...
            subject_mention = rc.get("subject_mention") or ""
            object_mention = rc.get("object_mention") or ""

            if not subject_mention or not object_mention:
                raise ContractError("Subject and object mentions cannot be empty.")

            # Both subject and object must be present in the evidence text (naïve check)
            if subject_mention.lower() not in extracted_text.lower() or object_mention.lower() not in extracted_text.lower():
                 raise ContractError("Subject and object mentions must both be found in the extracted evidence span.")
            ...
            claim = Claim(
                ...
                is_negative=str(rc.get("is_negative", False)).lower() == "true",
                is_speculative=str(rc.get("is_speculative", False)).lower() == "true"
            )
